...
- **Buku Besar & Drill-down**: Halaman "Buku Besar" (admin only) menampilkan timeline semua transaksi (PO/produksi/penjualan/beban) dengan running balance, filter tanggal+jenis, dan Ekspor CSV. Halaman Laporan: klik card Pendapatan/HPP/Beban Ops/Kas Masuk/Kas Keluar → modal berisi transaksi penyusun angka. Halaman Persediaan: tombol "Riwayat →" tiap SKU → modal berisi in (produksi) & out (penjualan) chronologis dengan saldo.
