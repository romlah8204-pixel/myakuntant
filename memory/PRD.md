...
- **POS Kasir + Role Granular + CSV Import + Alokasi Kanal (Feb 15, 2026)**:
  - **POS Kasir** (`/kasir`): halaman checkout cepat dengan kategori produk grid (auto price = HPP × 1,6), keranjang, edit qty/harga per baris, metode pembayaran (Tunai/QRIS/Kartu/Transfer), input pelanggan, checkout multi-item + tampilkan struk yang bisa dicetak (`window.print`). Sale disimpan dengan channel="POS" + payment_method. CSS khusus: `.pos-page/.pos-grid/.pos-cart/.pos-methods`.
